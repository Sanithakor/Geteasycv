      ...
      >
        ...
        <div className="space-y-4">
          ...
          {data.skills.length > 0 && (
            <div>
              <h3 className="text-[16.8px] font-bold uppercase tracking-wider leading-[1.25] opacity-90 mb-2">Skills</h3>
              <div className="flex flex-wrap gap-1.5">
                {data.skills.map((skill) => (
                  <span
                    key={skill.id}
                    className="text-[13.2px] font-semibold px-2 py-0.5 rounded-full bg-white/15 text-white"
                  >
                    {skill.name}
                  </span>
                ))}
          ...
          {data.certifications && data.certifications.length > 0 && (
            <div>
              <h3 className="text-[16.8px] font-bold uppercase tracking-wider leading-[1.25] opacity-90 mb-2">Certifications</h3>
              {data.certifications.map((cert) => (
                <div key={cert.id} className="mb-1.5">
                  <div className="text-[14.4px] font-bold leading-snug">{cert.name}</div>
                  <div className="text-[12.6px] opacity-80">{cert.issuer}</div>
                </div>
              ))}
            </div>
          )}
