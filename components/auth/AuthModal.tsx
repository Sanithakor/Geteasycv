...
const GITHUB_SVG = (
  <svg className="w-5 h-5 shrink-0 fill-current text-slate-900" viewBox="0 0 24 24" aria-hidden="true">
  ...
  const handleGithub = async () => {
    ...
    try {
      ...
      if (res.ok && result.success && result.user) {
        ...
        setError(result.error || 'GitHub sign-up failed. Please try again.');
      }
    } catch {
